...
class v8DetectionLoss:
    ...
    def _compute_attr_loss(
        ...
        Uses TAL assignment results to align each foreground anchor with its
        assigned GT box, then fetches that box's attribute multi-hot vector as
        ...
        # For each foreground anchor, look up its GT box's attribute vector.
        # target_gt_idx[b, a] is the within-image GT index for image b, anchor a.
        # We need to map it to a row in box_attr_mask, which is indexed globally.
        # Build a per-image offset: how many GT boxes come before image b.
        gt_counts = torch.zeros(batch_size, dtype=torch.long, device=attr_scores.device)
        for b in range(batch_size):
            gt_counts[b] = (batch_idx == b).sum()
        gt_offsets = gt_counts.cumsum(0) - gt_counts  # [B], offset for image b

        # Gather attribute targets for all foreground anchors
        # fg_mask: [B, n_anc]; iterate over batch to build (global_gt_idx, scores)
        fg_scores_list = []
        fg_targets_list = []
        for b in range(batch_size):
